...
def my_message_handler(message, payload):
    ...
    if message["type"] == "send":
        ...
        if my_json["my_type"] == "KEY":
            print("Key sent to SecretKeySpec()", payload.decode('utf-8').encode("hex"))
        elif my_json["my_type"] == "IV":
            print("Iv sent to IvParameterSpec()", payload.decode('utf-8').encode("hex"))
            ...
            print("Key sent to cipher init()", payload.decode('utf-8').encode("hex"))
        elif my_json["my_type"] == "IV from call to cipher init":
            print("Iv sent to cipher init()", payload.decode('utf-8').encode("hex"))
